...
@test "has java installed and it is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"java -version\""
  assert_output --partial 'openjdk version "1.8'
  assert_equal "$status" 0
}

@test "has correct environment variables set" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"env | grep JAVA\""
...
@test "has maven installed and it is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"mvn --version\""
